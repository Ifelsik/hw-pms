...
Reset_Handler:
  ...
  // тесты
  mov 	r10, #0 // результат (1 - fail, 0 - ok)
  mov   r11, #0 // номер упавшего теста

  // ошибка в значащем бите
  mov 	r0, #0b110001001101  // ошибочный код (0 в 6 бите (нумерация с 1))
  mov 	r2, #0b11001101  // ожидаемый
  bl	hamming_recover
  cmp 	r1, r2
  itt 	ne
  movne r10, #1
  ...
  // ошибка в контрольном бите
  mov 	r0, #0b110011101101  // ошибочный код (0 в 8 бите (нумерация с 1))
  mov 	r2, #0b11001101  // ожидаемый
  bl	hamming_recover
  cmp 	r1, r2
  itt	ne
  ...
  // передан корректный код
  mov 	r0, #0b110001101101
  mov 	r2, #0b11001101  // ожидаемый
  bl	hamming_recover
  cmp 	r1, r2
  itt	ne
...
// r0 - 12 знаков (8 значащих битов и 4 контрольных)
// r1 - результат исправления (8 бит, без контрольных)
// Алгоритм может исправить только 1 ошибочный бит!
hamming_recover:
  push 	{ r0, r2 - r4, lr }

  mov 	r1, r0

  mov 	r2, #0 // используется для синдрома
  mov 	r3, #1 // счётчик битов (в коде Хэмминга индексация с 1)
for_loop:
  tst 	r1, #1
  it 	ne // если 1 в младшем бите
  eorne	r2, r3

  lsr   r1, r1, #1

  add 	r3, #1  // инкремент счётчика бита

  cmp   r3, #13  // 13 бит? (индекс с 1)
  bne	for_loop // нет

  cmp	r2, #0  // синдром == 0?
  beq	extract_data

  sub 	r2, #1  // приведение к нормальной нумерации битов
  mov   r4, #1
  lsl	r4, r4, r2
  eor	r0, r4  // инверсия ошибочного бита по маске

// этот блок извлекает значащие биты
extract_data:
  // в коде хемминга с 8 значащими битами 4 служебных
  // значащие биты расположены по индексам:
  // 2, 4 - 6, 8 - 11.
